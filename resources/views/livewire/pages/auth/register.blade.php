...
new #[Layout('layouts.guest')] class extends Component
{
    ...
    public function register(): void
    {
        $validated = $this->validate([
            ...
            'role' => ['required', 'boolean'], // role alanını validate et

        ]);

        $validated['password'] = Hash::make($validated['password']);
        $validated['role'] = $this->role ? 1 : 0; // role 1 (admin) ya da 0 (kullanıcı) olarak ayarlanır.
...
<div>
    <form wire:submit="register">
        ...
        <!-- Role (Admin veya Kullanıcı Seçimi) -->
        <div class="mt-4">
            <x-input-label for="role" :value="__('')" />
            <input wire:model="role" type="checkbox" id="role" class="block mt-1" />
            <span>{{ __('Admin') }}</span>
